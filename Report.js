...
exports.getFullReport = async (req, res) => {
    ...
    try {
        ...
        // Check if the user is an advertiser or publisher
        const advertiser = await Advertiser.findOne({ userId });
        const publisher = await Publisher.findOne({ userId });
        ...
        if (advertiser) {
            ...
            return res.status(404).json({ message: 'User not found or does not have a valid role' });
        }

        // Process data based on the user's role
        if (userRole === 'advertiser') {
            // Fetch data for advertiser
            const totalClicks = userData.totalClicks;
            const totalImpressions = userData.totalImpressions;
            const totalCost = userData.totalCost;

            // Fetch campaign data for advertiser
            const campaigns = userData.campaigns;

            // Fetch website data for advertiser
            ...
            // Fetch data for publisher
            const totalClicks = userData.totalClicks;
            const totalImpressions = userData.totalImpressions;
            const totalRevenue = userData.totalRevenue;

            // Fetch campaignAssignment data for publisher
            const campaignAssignments = userData.campaignAssignments;

            // Fetch campaign data for publisher
            const campaigns = userData.campaigns;
